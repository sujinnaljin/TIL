...
- broker : 카프카 서버를 가리킴. n개의 브로커 중 1대는 컨트롤러 역할을 수행. 컨트롤러는 각 브로커에게 담당 파티션 할당을 수행하고 브로커가 정상 동작 하는지 모니터링.

- zookeeper : 카프카 서버 (+클러스터) 상태를 관리. 주키퍼에는 메타데이터(브로커id, 컨트롤러id)등이 저장 됨.
